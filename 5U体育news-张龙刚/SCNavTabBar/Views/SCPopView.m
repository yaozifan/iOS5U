...
- (NSArray *)getButtonsWidthWithTitles:(NSArray *)titles;
{
    ...
    for (NSString *title in titles)
    {
      ...
      CGSize size = [title boundingRectWithSize:CGSizeMake(WIDTH, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:20]} context:nil].size;
...
- (void)updateSubViewsWithItemWidths:(NSArray *)itemWidths;
{
    ...
    for (NSInteger index = 0; index < [itemWidths count]; index++)
    {
        ...
        @try {
            if ((buttonX + [itemWidths[index + 1]
                            
                            floatValue]) >= SCREEN_WIDTH)
            {
